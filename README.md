...
Answer: A token is an object which contains the security credentials for a login session and is used by servers to identify users, user's groups and sometimes user privileges. When a user logs on to a system, a token is generated by the server after the credentials provided by the user matches what is in the authentication database. The token is then attached to whatever process the user requires from that server and can be used by other processes. When the user makes another request that requires some authorization, the server compares the token attached in the request and grants access to the reesource only if the provided token matches the current token meant for that particular user.

- [ ] What steps can you take in your web apps to keep your data secure?

Answer: Data security can be improved in web applications through the use of tokens for particular requests from the server as well as through the use of private routes to limit access of certain sections of the web apps to only authorized users.

- [ ] Describe how web servers work.

Answer: Web servers act as storage devices connected to the internet for resources that are accessible world wide. When a client makes a request to the web server through a http request, the server retrieves the web page content of the code and sends it to the client.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

Answer: Create = Post method
        Read = Get method
        Update = Put method
